...
Claudeはauto_layout.pyで、text_overlay.mdの内容をイラストへ合成する（GPTが既に文字を描けている箇所は上書きしない）。

IMAGE_RULE.mdの納品前チェックリストで、機械的に確認できる項目（サイズ・空欄の有無等）は確認する。

**イラストの完成度・見た目の最終判断は人間が行う（2026-08-01変更）。**
AI同士（Claude・ChatGPT）はお互いの生成物の見た目の良し悪しを正確に判断するのが苦手であることが分かったため、
Claudeは機械的に確認できる項目のみをチェックし、デザイン面の完成度判断は人間の承認に委ねる。
人間が「これでよい」と判断したものは、そのまま次の工程に進めてよい。
...
レビューの対象は、キャプション・ハッシュタグ・コンプライアンス表現など**文章面**を中心とする。
イラストの見た目の完成度については、STEP3の人間承認をもって完了しているものとして扱う。
...
## 運用頻度について（2026-08-01追記）

Instagram投稿の作成は、人間からの都度の依頼（「GitHub最新版ルールを参照して作成してください」）に応じて進める。
Claude側から「次はどうしますか」「続けますか」と毎回連続で提案し続けることはしない。
1日分の作成・レビューが終わったら、そこで一区切りとし、次の指示を待つ。
